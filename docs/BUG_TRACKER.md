...
| B38 | FIXED      | **大 prompt 最后一层 MoE 0-token 崩溃**：`olmoe.cpp:124` 在 `il==n_layer-1` 用 `inp_out_ids` 把最后一层收窄到需输出 logits 的 token；非末尾 prefill ubatch 的 `n_outputs=0`，最后一层 MoE 的 ids 变成 `ne=[n_k,0,1]`。upstream 对 0-token op 是 no-op，但桶引擎 `build_mix_plan` 返回空 rounds → `chain_buckets empty round list L15` → `Compute error`（prompt > n_ubatch 必现；en.json < 512 不触发） | src/backend/minigraph_exec.cpp `exec_layer_burst_chain_buckets` | ✅ `n_t==0 || n_k==0` 直接返回 SUCCESS（等价 upstream 0-token no-op，KV 已由前面的 dense attention 写入）；olmoe place-cpu prefill3000 FAIL→OK（pp 208.9/tg 32.15），默认 ub512 与 `-ub 4096` 生成逐字节相同 |
| B39 | OPEN       | **gemma `C1:Vulkan0` + Vulkan 专家池 load 期 access violation**：`place-c1-exp2`/`place-c1c2-exp1`/`place-c1c2-exp2` 在装载阶段 `0xC0000005` 退出（非 OOM：`place-c1c2-exp5` 是干净 code=1 的 OOM）。规律：C1 在 Vulkan **且** 有 Vulkan 专家池才崩；C1 在 RAM 带池（place-c2-exp*、place-exp*）或 C1 在 Vulkan 不带池（place-c1、place-c1c2）都正常。**同样组合在 olmoe 上能 load**（RUN C en OK）→ gemma 特有（尺寸/分配边界），且是装载期不是计算期 | 疑：C1 上 GPU 重新打开 `op_offload`（llama.cpp 自分配 ~1.3G Vulkan compute buffer）与 route-B Vulkan 专家池/host-map 冲突；见 `docs/GRAPH_PARTITION.md` §7.2 接缝 | 待查：抓 load log；`C1:Vulkan0 + 池` 加 `--no-op-offload` 复测；检查 op_offload compute buffer 与池预留是否重叠 |
