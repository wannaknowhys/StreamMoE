...
| B39 | FIXED      | **gemma `C1:Vulkan0` + Vulkan 专家池 load 期 access violation**：`place-c1-exp2`/`place-c1c2-exp1`/`place-c1c2-exp2` 装载阶段 `0xC0000005`。**非显存**：128 MB 池 + `-ub 16` 仍崩。lldb 栈定位 `stream_moe_backend_replicate_leaf` 裸 `memcpy(dev, t->data, bytes)`：C4 leaf（gemma `ffn_down_exps.scale`）在 `C1:Vulkan0` 时落 Vulkan，`t->data` 非 host 可读 → AV。C1:RAM 时 leaf 在 host、无 Vulkan 池时复制循环为空，均不崩 | src/backend/moe_backend.cpp:478 `stream_moe_backend_replicate_leaf` | ✅ 改后端无关 `ggml_backend_tensor_get(t, dev, 0, bytes)`；原崩溃配置 c1-exp2/c1c2-exp2 均 exit=0。铁律（禁止 host 裸 memcpy 张量数据）见 `docs/GRAPH_PARTITION.md` §7.4 |
