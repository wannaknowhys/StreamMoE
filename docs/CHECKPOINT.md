...
- **v3chunk 引擎读取已通（B37 FIXED）**：多段时 `topo_builder` 曾按"每段"而非"每分支"建 SoA 列（列 stride 变成段长→相邻槽重叠覆盖）；改为按分支建列后，olmoe **1/2/3/5-chunk 对原版逐字节一致**（`prefill_export`+`expert_history`）。详见 `docs/BUG_TRACKER.md` B37。
