...
机制层操作对象是 **slot**，不是"专家的位置策略"：

- 目录记录 (L,E) → (pool, slot)；同一专家可同时持多个位置（非包含非独占只读副本）；
  scan 找第一个可用。
- 一个池 = 有界 slot 集，自带预算与**本地驱逐**：驱逐只因为**那个池满了**——在池内找谁
  出去；绝不做跨池的"专家好坏"判断。
- 原语：往某 slot 载内容（DIO 进 RAM/VRAM）、槽内容在池间移（RAM↔VRAM；无 VRAM↔VRAM——
  GPU 间搬不值，重读/DIO 代替）、释放 slot（逐该副本）。
- 副本让"逐出"变"逐副本"：每池逐自己的 slot。
...
- **M1** — CPU-only 私有链骨架：每层私有 mini-graph（gemma arch 的拓扑重建器；官方内核、
  中间在我们 arena、只 cur/moe_out 碰主图）+ 两个伪 device_pool（都 CPU、独立池+执行区），
  钉死 ids 分组/每池子集链/汇总逻辑与数值==官方（v2 对齐）。定义自调度器接口与链拓扑重建器。
