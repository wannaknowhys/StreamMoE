...
1. **单 prompt 优先**：任何改动后，先用 `build\main\llama-build\bin\llama-cli.exe -m <model> --expert-backend --moe-ram-pool <MB> --fit off --no-warmup -p "..." -n 32` 直接观察返回文本与行为，**不要**直接跑 server + 长测试。
...
4. **后台进程必须可回收**：跑批前后 `taskkill /F /IM llama-server.exe`，杜绝僵尸占端口。
...
```bat
build\main\llama-build\bin\llama-cli.exe -m "N:\AI_LLM\DeepSeek-V4-Flash-0731\DeepSeek-V4-Flash-0731-UD-Q8_K_XL-00001-of-00005.gguf" --expert-backend --moe-ram-pool 71680 -c 8192 -t 16 --fit off --no-warmup -p "Say hello in one short sentence." -n 32
```
...
产物：`benchmark\results\conversation_real_<tag>.txt` / `profile_real_<tag>.jsonl`；server 日志 `build\<btag>\server_<tag>.log`。

**用户职责**：盯内存；异常立即关窗/杀 `llama-server.exe`。
...
- **端口冲突**：跑批前先 `taskkill /F /IM llama-server.exe`。
